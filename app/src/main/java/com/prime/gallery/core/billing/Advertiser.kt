package com.prime.gallery.core.billing
...
fun Advertiser(
    ...
    override fun show(
        activity: Activity, force: Boolean, action: (() -> Unit)?
    ) {
...
private val iBannerListener = object : BannerView.IListener {
    ...
    override fun onBannerFailedToLoad(bannerView: BannerView, errorInfo: BannerErrorInfo) {
        fAnalytics.logEvent(
            "Banner failed load: id = ${bannerView.placementId} error: ${errorInfo.errorCode}", null
        )
        //bannerView.load()
    }
