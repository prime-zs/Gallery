...
private inline fun Horizontal(
    ...
    Layout(
        ...
        measurePolicy = { measurables, constraints ->
            ...
            layout(width, height) {
                var x: Int = navRailBarPlaceable?.width ?: 0
                var y: Int = 0
                contentPlaceable.placeRelative(x, y)
                // Place toaster at the centre bottom of the screen
                // remove nav bar offset from it.
                x = (navRailBarPlaceable?.width ?: 0) + (contentPlaceable.width / 2) - channelPlaceable.width / 2   // centre
                // full height - toaster height - navbar - 16dp padding + navbar offset.
...
fun Scaffold(
    ...
    val composed =
        @Composable {
            ...
            // The SnackBar
            SnackbarHost(hostState = channel)
